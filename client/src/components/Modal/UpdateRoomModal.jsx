  ...
  const handleImage = async(files) => {
    ...
    try {
      ...
      for (let i = 0; i < filesArray.length; i++) {
        const image_url = await imageUpload(filesArray[i]);
        uploadedUrls.push(image_url);
        console.log(`Uploaded image ${i + 1}:`, image_url);
      }
      
      // Update roomData with images array
      setRoomData({...roomData, images: uploadedUrls});
