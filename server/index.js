...
async function run() {
  try {
    ...
    const usersCollection = client.db('hotelBooking').collection('users')
...
app.get('/rooms', async (req, res) => {
  const { category, bed_type, location } = req.query
  ...
  if (bed_type && bed_type.trim() !== '') {
    ...
    query.bed_type = { $regex: bed_type, $options: 'i' }
  }
...
//     sendMail(bookingData?.host?.email, {
//       subject: "Room Booked", 
//       message: `Your room has been booked by ${bookingData?.guest?.name} (${bookingData?.guest?.email}). Transaction ID: ${bookingData?.transactionId}`
//     });
